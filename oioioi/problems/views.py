...
def add_or_update_problem_view(request):
    return add_or_update_problem(request, request.contest, "problems/add-or-update.html")
...
def filter_problems_by_origin(problems, origin_tags):
    ...
    for tag in origin_tags:
        ...
        if len(tag) in (1, 2):
            ...
            if len(tag) == 2:
                value = OriginInfoValue.objects.filter(parent_tag__name=tag[0], value=tag[1])
                if not value.exists():
                    raise Http404
                value = value.get()
...
def _get_problems_by_query(query):
    prefetch = Prefetch("names", queryset=ProblemName.objects.filter(language=get_language()))
    filtered_problems = Problem.objects.prefetch_related(prefetch).filter(
        Q(names__name__icontains=query) | Q(legacy_name__icontains=query) | Q(ascii_name__icontains=unidecode(str(query))) | Q(short_name__icontains=query),
...
def filter_problems_by_query(problems, datadict):
    ...
    if algorithm_tags:
        if settings.SHOW_TAG_PROPOSALS_IN_PROBLEMSET and datadict.get("include_proposals", "0") == "1":
            ...
            problems = problems.filter(Q(algorithmtag__name__in=algorithm_tags) | Q(id__in=proposal_match_problem_ids)).distinct()
        else:
...
def problemset_get_problems(request):
    ...
    if settings.PROBLEM_STATISTICS_AVAILABLE:
        ...
        if request.user.is_authenticated:
            user_statistics = UserStatistics.objects.filter(problem_statistics=OuterRef("statistics__pk"), user=request.user)
            ...
            problems = problems.annotate(
                ...
                user_statistics_has_submitted=Coalesce(Subquery(user_statistics.values("has_submitted")[:1]), False),
                user_statistics_has_solved=Coalesce(Subquery(user_statistics.values("has_solved")[:1]), False),
            ).annotate(
            ...
            filter_types = {
                ...
                "attempted": Q(user_statistics_has_solved=False) & Q(user_statistics_has_submitted=True),
    ...
    if "order_by" in request.GET:
        ...
        if field == "name":
            problems = problems.annotate(localized_name=Subquery(ProblemName.objects.filter(problem=OuterRef("pk"), language=get_language()).values("name")))
...
def problemset_generate_view(request, page_title, problems, view_type):
    ...
    difficulty_tags = DifficultyTag.objects.filter(name__in=request.GET.getlist("difficulty")).order_by("name")
...
def problem_site_view(request, site_key):
    ...
    difficulty_options = (tag.full_name for tag in DifficultyTagLocalization.objects.filter(language=get_language()).order_by("full_name"))
    context = {
        "problem": problem,
...
def get_report_HTML_view(request, submission_id):
    ...
    if not controller.filter_my_visible_submissions(request, Submission.objects.filter(id=submission_id)).exists():
        raise Http404
    reports = ""
    ...
    if not reports:
        reports = _("Reports are not available now (ಥ ﹏ ಥ)")
        reports = mark_safe("<center>" + force_str(reports) + "</center>")
...
def get_report_row_begin_HTML_view(request, submission_id):
    ...
    return TemplateResponse(
        ...
        "contests/my_submission_table_base_row_begin.html",
        {
            ...
            "can_admin": can_admin_problem_instance(request, submission.problem_instance) and json.loads(request.POST.get("can_admin", "false")),
...
def problemset_add_or_update_problem_view(request):
    if not can_add_to_problemset(request):
        if request.contest and not request.contest.is_archived:
            url = reverse("add_or_update_problem") + "?" + urllib.parse.urlencode(request.GET.dict())
            return safe_redirect(request, url)
        raise PermissionDenied

    return add_or_update_problem(request, None, "problems/problemset/add-or-update.html")


def task_archive_view(request):
    origin_tags = OriginTag.objects.all().prefetch_related("localizations").order_by("name")
...
def _recursive_group_problems(problems, result_info, categories, div_id):
    if not categories:
        ...
        percentages = [100.0 * result["score"] / result["max_score"] if result["exists"] else 0.0 for result in results]
        ...
        return {
            ...
            "subnodes": [],
            "problem_info": zip(problems, results, strict=False),
    ...
    iter = groupby(problems, key=lambda problem: get_prefetched_value(problem, category))
    ...
    for value, group in iter:
        ...
        child = _recursive_group_problems(child_problems, result_info, categories[1:], child_id)
    ...
    for problem in problems:
        ...
        if result is None:
            ...
            try:
                ...
                def result_info(_int_score, _int_max_score, _submission_id):
                    return {
                        "exists": True,
                        "score": _int_score,
                        ...
                        "submission_url": reverse(
                            "submission",
                            kwargs={"submission_id": _submission_id},
                        ),
                    }

                if score is None and max_score is None:
                    if result.status is None or result.status == "?" or result.status == "INI_OK" or result.status == "INI_ERR":
                        results_info[problem] = {"exists": False}
                    elif result.status == "OK":
                        results_info[problem] = result_info(1, 1, submission_id)
                    else:
                        results_info[problem] = result_info(0, 1, submission_id)
                else:
                    results_info[problem] = result_info(score.to_int(), max_score.to_int(), submission_id)
...
def task_archive_tag_view(request, origin_tag):
    ...
    problems = (
        ...
        .prefetch_related("origininfovalue_set__localizations", "origininfovalue_set__category", "names")
    )
    ...
    categories = sorted([cat for cat in categories if cat.order], key=lambda cat: cat.order)
    ...
    problems_root_node = _recursive_group_problems(problems, user_results, categories, "problemgroups")
    ...
    return [
        {
            ...
            "name": f"{origintag.full_name} - {category.full_name}",
...
def get_origininfovalue_hints(query):
    ...
    return (
        ...
        + [
            ...
            for oivl in OriginInfoValueLocalization.objects.filter(full_value__istartswith=query)
        ]
        + (
            [
                {
...
def get_selected_origintag_hints_view(request):
    result = _get_origintag_hints(OriginTagLocalization.objects.filter(full_name__iexact=request.GET.get("q")))
...
def get_origininfocategory_hints_view(request):
    tag = get_object_or_404(OriginTagLocalization, language=get_language(), full_name=request.GET.get("q")).origin_tag
    category = get_object_or_404(OriginInfoCategory, parent_tag=tag, name=request.GET.get("category"))
    ...
    return [
        {
            "trigger": "origininfo",
            "name": f"{category.parent_tag.full_name} {val.full_value}",
            "prefix": "origin",
            "value": val.name,
        }
...
def get_algorithm_tag_proposal_hints_view(request):
    ...
    result = []
    result.extend([tag.full_name for tag in AlgorithmTag.objects.filter(name__icontains=query)])
    result.extend([tag.algorithm_tag.full_name for tag in AlgorithmTagLocalization.objects.filter(full_name__icontains=query)])
...
def save_proposals_view(request):
    if request.method == "POST":
        ...
        difficulty_tag_localization = DifficultyTagLocalization.objects.filter(full_name=difficulty, language=get_language()).first()
        ...
        difficulty_tag_proposal = DifficultyTagProposal(problem=problem, tag=difficulty_tag, user=user)
...
def download_problem_package_file_view(request, package_id, file_name):
    ...
    try:
        ...
        if os.path.isfile(filepath):
            return stream_file(File(open(filepath)), os.path.basename(os.path.normpath(file_name)))
    finally:

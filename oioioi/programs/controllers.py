...
class ProgrammingProblemController(ProblemController):
    ...
    def get_compiler_for_submission(self, submission):
        ...
        compiler = problem_instance.controller.get_compiler_for_language(problem_instance, language)
    ...
    def generate_initial_evaluation_environ(self, environ, submission, **kwargs):
        ...
        environ["compilation_result_size_limit"] = problem_instance.controller.get_compilation_result_size_limit(submission)
        ...
        environ["contest_weight"] = contest.judging_weight if contest is not None else settings.NON_CONTEST_WEIGHT
    ...
    def generate_base_environ(self, environ, submission, **kwargs):
        ...
        environ.setdefault("error_handlers", []).append(("delete_executable", "oioioi.programs.handlers.delete_executable"))

        if getattr(settings, "USE_UNSAFE_EXEC", False):
            environ["exec_mode"] = "unsafe"
        else:
            environ["exec_mode"] = submission.problem_instance.controller.get_safe_exec_mode()
            ...
            recipe_body.extend(
                [
                    ...
                    (
                        ...
                        "oioioi.programs.handlers.fill_outfile_in_existing_test_reports",
                    ),
                    (
    ...
    def _map_report_to_submission_status(self, status, problem_instance, kind="INITIAL"):
        if kind == "INITIAL":
    ...
    def update_submission_score(self, submission):
        ...
        try:
            report = SubmissionReport.objects.filter(submission=submission, status="ACTIVE", kind=kind_for_status).get()
    ...
    def check_repeated_submission(self, request, problem_instance, form):
        return not can_admin_problem(request, problem_instance.problem) and form.kind == "NORMAL" and getattr(settings, "WARN_ABOUT_REPEATED_SUBMISSION", False)
    ...
    def validate_submission_form(self, request, problem_instance, form, cleaned_data):
        ...
        if (not is_file_chosen and not is_code_pasted) or (is_file_chosen and is_code_pasted):
        ...
        if not cleaned_data[langs_field_name] and is_file_chosen:
            ...
            cleaned_data[langs_field_name] = get_language_by_extension(problem_instance, ext)
        ...
        if cleaned_data[langs_field_name] not in langs.keys():
            raise ValidationError(_("This language is not allowed for selected problem."))
        ...
        if controller.check_repeated_submission(request, problem_instance, form):
            ...
            if session_md5_key in request.session and md5 == request.session[session_md5_key]:
                ...
                raise ValidationError(_("You have submitted the same file for this problem again. Please resubmit if you really want to submit the same file"))
            else:
    ...
    def create_submission(self, request, problem_instance, form_data, judge_after_create=True, **kwargs):
        submission = ProgramSubmission(
            ...
            kind=form_data.get(
                "kind",
                problem_instance.controller.get_default_submission_kind(request, problem_instance=problem_instance),
            ),
    ...
    def adjust_submission_form(self, request, form, problem_instance):
        ...
        form.fields["file"].widget.attrs.update({"data-languagehintsurl": reverse("get_language_hints")})
        ...
        if use_editor:
            ...
            code_widget = forms.widgets.Textarea(attrs={"rows": 10, "class": "monospace"})

        form.fields["code"] = forms.CharField(required=False, label=_("Code"), widget=code_widget)
        ...
        if "dropped_solution_name" in request.POST:
            ...
            if fname.count(".") == 1:
                ...
                if langs_field_name not in request.POST:
                    form.fields[langs_field_name].initial = get_language_by_extension(problem_instance, ext)
    ...
    def render_submission(self, request, submission):
        ...
        if submission.kind == "USER_OUTS":
            ...
            submission.programsubmission.comment = mark_safe(submission.programsubmission.comment)
        ...
        return render_to_string(
            ...
            request=request,
            context={
                "submission": submission_template_context(request, submission.programsubmission),
                "problem": get_problem_link_or_name(request, submission),
                "saved_diff_id": request.session.get("saved_diff_id"),
                "supported_extra_args": problem_instance.controller.get_supported_extra_args(submission),
                "can_admin": can_admin,
    ...
    def can_see_source(self, request, submission):
        ...
        return request.user.is_superuser or self.filter_my_visible_submissions(request, qs).exists()
    ...
    def _out_generate_status(self, request, testreport):
        ...
        try:
            if can_admin_problem(request, problem) or testreport.userout_status.visible_for_user:
                # making sure, that output really exists or is processing
                if bool(testreport.output_file) or testreport.userout_status.status == "?":
    ...
    def get_allowed_languages(self):
        ...
        main_languages_only = [lang for lang, meta in all_languages.items() if meta["type"] == "main"]
...
class ProgrammingContestController(ContestController):
    ...
    def _map_report_to_submission_status(self, status, problem_instance, kind="INITIAL"):
        return problem_instance.problem.controller._map_report_to_submission_status(status, problem_instance, kind)

    def get_compilation_result_size_limit(self, submission):
        return submission.problem_instance.problem.controller.get_compilation_result_size_limit(submission)
    ...
    def get_submission_size_limit(self, problem_instance):
        return problem_instance.problem.controller.get_submission_size_limit(problem_instance)

    def check_repeated_submission(self, request, problem_instance, form):
        return not is_contest_basicadmin(request) and form.kind == "NORMAL" and getattr(settings, "WARN_ABOUT_REPEATED_SUBMISSION", False)
    ...
    def can_see_source(self, request, submission):
        ...
        """
        qs = Submission.objects.filter(id=submission.id)
        if not (is_contest_admin(request) or is_contest_observer(request)) and is_model_submission(submission):
            return False
    ...
    def _out_generate_status(self, request, testreport):
        try:
            if is_contest_basicadmin(request) or testreport.userout_status.visible_for_user:
                # making sure, that output really exists or is processing
                if bool(testreport.output_file) or testreport.userout_status.status == "?":
                    return testreport.userout_status.status
    ...
    def can_see_test_comments(self, request, submissionreport):
        return submissionreport.submission.problem_instance.problem.controller.can_see_test_comments(request, submissionreport)

...
def extract_code(show_response):
    ...
    show_response.content = strip_tags(six.ensure_text(show_response.content[open_pre_end:close_pre_start], errors="replace"))
...
def extract_code_from_diff(show_response):
    ...
    # Get substring and strip tags.
    show_response.content = strip_tags(six.ensure_text(show_response.content[pre_start:pre_end], errors="replace"))
...
class TestProgramsViews(TestCase, TestStreamingMixin):
    ...
    def test_submission_views(self):
        ...
        show_response = self.client.get(reverse("show_submission_source", kwargs=kwargs))
        self.assertEqual(show_response.status_code, 200)
        # Download plain text response.
        download_response = self.client.get(reverse("download_submission_source", kwargs=kwargs))
        # Extract code from <pre>'s
        extract_code(show_response)
        # Shown code has entities like &gt; - let's escape the plaintext.
        download_response_content = escape(self.streamingContent(download_response).decode("utf-8"))
        # Now it should work.
...
class TestSourceWithoutContest(TestCase):
    ...
    def test_source_permissions(self):
        ...
        self.assertTrue(self.client.login(username="test_user"))
        check_is_accessible(self, "show_submission_source", kwargs={"submission_id": submission.id})

        self.assertTrue(self.client.login(username="test_admin"))
        check_is_accessible(self, "show_submission_source", kwargs={"submission_id": submission.id})

        self.assertTrue(self.client.login(username="test_user2"))
        check_not_accessible(self, "show_submission_source", kwargs={"submission_id": submission.id})
...
class TestHeaderLinks(TestCase):
    ...
    def test_link_to_changelist_visibility(self):
        ...
        self.assertContains(
            response,
            f'<a href="{link_url}">{submission.problem_instance}</a>',
            html=True,
        )
        ...
        self.assertTrue(self.client.login(username="test_user"))
        response = self.client.get(reverse("submission", kwargs=kwargs))
        self.assertNotContains(response, f'<a href="{link_url}">', html=True)
...
class TestProgramsXssViews(TestCase, TestStreamingMixin):
    ...
    def test_submission_xss_views(self):
        ...
        self.assertTrue(download_response.streaming)
        self.assertEqual(show_response.content.decode("utf-8"), download_response_content)
        self.assertNotContains(show_response, "<script>")
        self.assertNotContains(diff_response, "<script>")
        self.assertNotIn(download_response_content, "<script>")
...
class TestSubmission(TestCase, SubmitFileMixin):
    ...
    def test_submission_completed_notification(self):
        ...
        Round.objects.update(results_date=django_timezone.now() + timedelta(minutes=30))
        send_notification_judged(environ)
        self.assertEqual(len(messages), 1)
    ...
    def test_pasting_unicode_code(self):
        contest = Contest.objects.get()
        problem_instance = ProblemInstance.objects.get(pk=1)
        response = self.submit_code(contest, problem_instance, chr(12345), user="test_user")
        self._assertSubmitted(contest, response)
...
class TestSubmittingAsContestAdmin(TestCase):
    ...
    def test_missing_permission(self):
        contest = Contest.objects.get()
        contest.controller_name = "oioioi.programs.tests.PrivateProgrammingContestController"
        contest.save()
        pi = ProblemInstance.objects.get()
        url = reverse("submit", kwargs={"contest_id": contest.id})
...
class TestScorers(TestCase):
    ...
    def test_discrete_test_scorer(self):
        ...
        expected = list(zip(exp_scores, exp_max_scores, exp_statuses, strict=False))

        results = list(map(utils.discrete_test_scorer, *list(zip(*self.t_results_ok, strict=False))))
        self.assertEqual(expected, results)

        results = list(map(utils.discrete_test_scorer, *list(zip(*self.t_results_wrong, strict=False))))
        self.assertEqual(self.t_expected_wrong, results)

        results = list(map(utils.discrete_test_scorer, *list(zip(*self.t_results_unequal_max_scores, strict=False))))
    ...
    def test_threshold_linear_test_scorer(self):
        ...
        results = list(map(utils.threshold_linear_test_scorer, *list(zip(*self.t_results_ok, strict=False))))
        self.assertEqual(expected, results)
        ...
        results = list(map(utils.threshold_linear_test_scorer, *list(zip(*self.t_results_ok_perc, strict=False))))
        self.assertEqual(expected, results)
        ...
        self.assertEqual(utils.threshold_linear_test_scorer(*malformed), (0, 100, "TLE"))
    ...
    def test_sum_group_scorer(self):
        ...
        self.assertEqual((359, 630, "WA"), utils.sum_group_scorer(self.g_results_unequal_max_scores))

    def test_sum_score_aggregator(self):
        self.assertEqual((349, 400, "OK"), utils.sum_score_aggregator(self.g_results_ok))
        self.assertEqual((349, 600, "WA"), utils.sum_score_aggregator(self.g_results_wrong))
        self.assertEqual(
            (359, 630, "WA"),
...
class TestUserOutsGenerating(TestCase):
    ...
    def test_report_after_generate(self):
        ...
        ReportActionsConfig(problem=submission.problem_instance.problem, can_user_generate_outs=True).save()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_generate_and_download_user_permission(self):
        ...
        down_all_url = reverse("download_user_output", kwargs={"submission_report_id": 2})
...
class TestRejudge(TestCase, SubmitFileMixin):
    ...
    def _test_rejudge(
        ...
        reports = TestReport.objects.filter(submission_report__submission=submission, submission_report__status="ACTIVE")
        for r in reports:
...
class TestLimitsLimits(TestCase):
    ...
    def test_time_limit(self):
        ...
        self.assertContains(
            response,
            "Sum of time limits for all tests is too big. It's 7s, but it shouldn't exceed 6s.",
            html=True,
        )
...
class TestCompiler(TestCase):
    ...
    def test_contest_admin_inline(self):
        ...
        contest = Contest.objects.get()
        url = reverse("oioioiadmin:contests_contest_change", args=(quote(contest.id),)) + "?simple=true"
    ...
    @override_settings(SUBMITTABLE_EXTENSIONS={"C": ["c"], "Python": ["py"]})
    @override_settings(AVAILABLE_COMPILERS={"C": {"gcc": {"display_name": "gcc"}, "clang": {"display_name": "clang"}}})
    @override_settings(DEFAULT_COMPILERS={"C": "gcc", "Python": "python"})
...
class TestMaxScoreMigration(TestCaseMigrations):
    ...
    def make_report(self, problem_id, contest, apps, max_score):
        ...
        submission_report = submission_report_model.objects.create(submission=submission)

        test_report = test_report_model.objects.create(submission_report=submission_report, test_max_score=max_score, time_used=1)
    ...
    def setUpBeforeMigration(self, apps):
        ...
        default_contest = contest_model.objects.create(id=1, controller_name="oioioi.contests.controllers.ContestController")
        pa_contest = contest_model.objects.create(id=2, controller_name="oioioi.pa.controllers.PAContestController")
        acm_contest = contest_model.objects.create(id=3, controller_name="oioioi.acm.controllers.ACMContestController")
    ...
    def test(self):
        ...
        self.assertEqual(test_report_model.objects.get(pk=self.pa_report_id).max_score.to_int(), 1)
        self.assertEqual(
            test_report_model.objects.get(pk=self.pa_report_zero_id).max_score.to_int(),
            0,
        )

        self.assertTrue(test_report_model.objects.get(pk=self.acm_report_id).max_score is None)
...
class TestAllowedLanguages(TestCase, SubmitFileMixin):
    ...
    def test_disallowed_language_submit_attempt(self):
        ...
        response = self.submit_code(contest, self.problem_instance, "some code", prog_lang="C")
        self._assertSubmitted(contest, response)
...
class TestLanguageOverrideForTest(TestCase):
    ...
    def test_proper_env_override(self):
        ...
        LanguageOverrideForTest.objects.create(test=tests[0], time_limit=1, memory_limit=2, language="cpp")
        ...
        self.assertEqual(env_with_tests["tests"]["1a"]["exec_time_limit"], tests[1].time_limit)
        self.assertEqual(env_with_tests["tests"]["1a"]["exec_mem_limit"], tests[1].memory_limit)
    ...
    def test_format(self):
        ...
        for format_contest, format_problem, expected in [
            ...
            if format_problem is not None:
                CheckerFormatForProblem.objects.create(problem_instance=pi, format=format_problem)
